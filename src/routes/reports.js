...
async function generateNarrative(leader, scoreData, commentData) {
  ...
  const flags = [
    ...
    scoreData.hiddenStrengths.length ? `HIDDEN STRENGTHS: ${scoreData.hiddenStrengths.map(h=>`${h.section} (${h.group} rates higher than self)`).join('; ')}` : null,
    scoreData.highScores.length ? `HIGH SCORES (4.4+): ${scoreData.highScores.map(h=>`${h.section} from ${h.group}`).join('; ')}` : null,
    scoreData.lowScores.length ? `LOW SCORES (3.5 or below): ${scoreData.lowScores.map(l=>`${l.section} from ${l.group}`).join('; ')}` : null,
  ].filter(Boolean).join('\n');
  ...
  const prompt = `You are a skilled leadership development coach generating a CARE 360 feedback report for In Good Company Collective — a boutique talent and leadership development consultancy.

LEADER: ${leader.name}${leader.title ? ', ' + leader.title : ''}

THE CARE FRAMEWORK (this report is organized around these four pillars plus Leadership Effectiveness):
...
YOUR TASK: Generate a narrative feedback report that helps this leader feel SEEN in a positive way, understand their data clearly, and feel genuinely motivated to grow. This is NOT a gap analysis exercise. Strengths are the foundation. Growth is an invitation, not a verdict.

CRITICAL PRINCIPLES:
1. Lead with what is genuinely working before addressing development areas
2. Be specific — reference actual score patterns and comment themes, not generic leadership advice
3. Name divergences between rater groups when they are meaningful (e.g. supervisor sees X differently than direct reports)
4. Blind spots need care — surface them honestly but constructively
5. The tone should feel like a trusted coach, not a performance review
6. Never use generic phrases like "continue to leverage strengths" or "areas for improvement" — be direct and specific
7. Write in full, warm, direct sentences. No bullet points in the narrative fields.

Return ONLY a valid JSON object with this exact structure — no markdown fences, no preamble:
{
  "keyInsight": "The single most important pattern across ALL the data in one specific, direct sentence. This goes at the top of the report. Make it something the leader will recognize immediately.",
  "overview": "3-4 sentences. What is the overall picture? What is this leader's greatest strength as experienced by others? Where is the clearest opportunity? Note any significant pattern across rater groups (e.g. supervisor sees them very differently from their team). Warm, direct, specific.",
  "sections": {
    "connect": {
      "whatsWorking": "2-3 sentences. What do raters see this leader doing well in this pillar? Ground it in the scores and comments. Be specific.",
      "growthEdge": "2-3 sentences. What is the development opportunity here? If scores are all high and comments are positive, say so honestly rather than inventing an opportunity. If there is a real gap, name it directly but constructively.",
      "raterPattern": "1-2 sentences. Any notable difference between how different groups experience this leader in this area? Skip if no meaningful divergence."
    },
    "accountable": { "whatsWorking": "...", "growthEdge": "...", "raterPattern": "..." },
    "reach": { "whatsWorking": "...", "growthEdge": "...", "raterPattern": "..." },
    "empower": { "whatsWorking": "...", "growthEdge": "...", "raterPattern": "..." },
    "effectiveness": { "whatsWorking": "...", "growthEdge": "...", "raterPattern": "..." }
  },
  "sscThemes": "2-3 sentences synthesizing the Start/Stop/Continue responses into themes. What do raters most want to see more of, less of, or kept? Skip if no SSC data.",
  "closingReflection": "2-3 sentences. A direct, warm closing statement that affirms this leader's capacity to grow from this feedback. Reference something specific from the data. Should feel like the coach's final word before handing the report to the leader."
}`;
  ...
  try {
    ...
    console.error('Narrative parse error:', e.message);
    return { keyInsight:'', overview:'', sections:{}, sscThemes:'', closingReflection:'' };
  }
}
