...
router.get('/generate/:leaderId', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Custom questions belong to the leader's Group (cycle), and are
    // optional. Everything below is skipped entirely when a Group has
    // none, so a leader with no custom questions produces exactly the
    // same report as before this feature existed.
    const { data: customQuestions } = await supabase
      .from('custom_questions').select('*').eq('cycle_id', leader.cycle_id).order('position');
    ...
    if (customQuestions && customQuestions.length) {
      ...
      const { data: customComments } = await supabase
        .from('custom_comments').select('*, raters(rater_group)').eq('leader_id', leaderId);
    ...
    const reportRow = {
      ...
      report_data: { scoreData, narrative, customSection },
      generated_by: 'ai'
    };
...
// ═══════════════════════════════════════════════════════════════
// CUSTOM QUESTIONS — scoring and comments
//
// Deliberately mirrors buildScoreData/buildCommentData in shape, but
// stays completely separate. Custom questions are combined into one
// score per rater group, the same way a single CARE section combines
// its own questions, since that is the unit the report shows a score
// for. Nothing here ever touches scoreData or commentData.
// ═══════════════════════════════════════════════════════════════
function buildCustomScoreData(customResponses) {
  const groups = ['self','supervisor','peer','direct_report','skip_level'];
  const byGroup = {}; groups.forEach(g => { byGroup[g] = []; });

  (customResponses || []).forEach(r => {
    const group = r.raters?.rater_group;
    if (!group || !byGroup[group]) return;
    if (typeof r.score === 'number') byGroup[group].push(r.score);
  });

  const avg = arr => (!arr || !arr.length) ? null : Math.round((arr.reduce((a,b)=>a+b,0)/arr.length)*100)/100;
  const scores = {}; groups.forEach(g => { scores[g] = avg(byGroup[g]); });
  return scores;
}

function buildCustomCommentData(customComments) {
  return (customComments || [])
    .filter(c => c.response)
    .map(c => ({ group: c.raters?.rater_group || 'unknown', text: c.response }));
}
...
// Shared by both the CARE narrative and the custom question narrative,
// so the two prompts can use the same delimiter format without
...
2-3 sentences affirming this leader's capacity to grow from this feedback. Reference something specific from the data. This should feel like a coach's final word before handing the leader their report.`;
  ...
  const raw = stripDashes(response.content[0].text.trim());
  console.log('Narrative response length:', raw.length);

  const get = (marker) => extractMarker(raw, marker);
...
// ═══════════════════════════════════════════════════════════════
// CUSTOM QUESTION NARRATIVE
//
// A separate, smaller call with no CARE framework in it at all. This
// is deliberate. A custom question about a client's own value or
// initiative has no relationship to the validated CARE constructs,
...
async function generateCustomNarrative(leader, customQuestions, customScores, customComments) {
  ...
  const questionList = customQuestions.map((q, i) => `${i + 1}. ${q.rater_text}`).join('\n');
  const commentText = customComments.length
...
PRINCIPLES:
- Ground everything in the actual scores and comments given. Do not speculate beyond them.
- Tone should feel like a trusted coach, not a performance review.
- Write in full, warm, direct sentences only.
- Never use em dashes or en dashes. Use commas, colons, periods, or restructure the sentence instead. This is a strict requirement.
...
##CUSTOM_WORKING##
2-3 sentences on what the data shows is going well here.
##CUSTOM_GROWTH##
2-3 sentences on the growth edge, if the data points to one. Write NONE if the data does not support a clear growth edge.
##CUSTOM_PATTERN##
1-2 sentences on any notable difference between rater groups. Write NONE if there is no meaningful divergence.`;
  ...
  return {
    whatsWorking: get('CUSTOM_WORKING'),
    growthEdge:   get('CUSTOM_GROWTH'),
    raterPattern: get('CUSTOM_PATTERN'),
  };
}
...
// ═══════════════════════════════════════════════════════════════
function buildReportHtml(leader, scoreData, narrative, commentData, customSection) {
  const RATER_COLORS = {
  ...
  // Custom questions section. Reuses every class the five CARE sections
  // already use, so it inherits the exact same pagination behavior
  // without needing a single new CSS rule. What sets it apart visually
  // is the heading itself: no "Section N" eyebrow, and language that
  // never implies it is a sixth CARE dimension.
  function customSectionBlock() {
    if (!customSection) return '';
    const narr     = customSection.narrative || {};
    const comments = (customSection.comments || []).filter(c => c.text);
...
<div class="cover">
  <div style="margin-bottom:28px"><img src="data:image/png;base64,PLACEHOLDER" alt="In Good Company" style="height:48px"/></div>
